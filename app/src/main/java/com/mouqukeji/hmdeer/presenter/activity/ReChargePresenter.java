...
public class ReChargePresenter extends ReChargeContract.Presenter {
    ...
    @Override
    public void reChargePage(MultipleStatusView multipleStatusView) {
        rxManager.addObserver(RetrofitManager.getInstance().doRequest(mModel.reChargePage(), new RxObserverListener<RechargePageBean>(mView) {
            @Override
            public void onSuccess(RechargePageBean result) {

                mView.reChargePage(result);
            }
        }));
    }
}
