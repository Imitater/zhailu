...
import com.mouqu.zhailu.zhailu.contract.fragment.CompleteOrderContract;
import com.mouqu.zhailu.zhailu.contract.fragment.OngingOrderContract;
import com.mouqu.zhailu.zhailu.modle.fragment.CompleteOrderModel;
import com.mouqu.zhailu.zhailu.modle.fragment.OngoingOrderModel;
import com.mouqu.zhailu.zhailu.presenter.fragment.CompleteOrderPresenter;
import com.mouqu.zhailu.zhailu.presenter.fragment.OngoingOrderPresenter;
import com.mouqu.zhailu.zhailu.ui.adapter.CompleteOrderRecyclerviewAdapter;
        ...
        orderRecycler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (mCurrentCounter >= countBean.size() && page < 2) {
                    ...
                    if (isErr) {
                        //成功获取更多数据
                        completeOrderRecyclerviewAdapter.addData(countBean);
                        mCurrentCounter = completeOrderRecyclerviewAdapter.getData().size();
                        completeOrderRecyclerviewAdapter.loadMoreComplete();
                    } else {
                        //获取更多数据失败
                        isErr = true;
                        Toast.makeText(getMContext(), "数据加载失败", Toast.LENGTH_LONG).show();
                        completeOrderRecyclerviewAdapter.loadMoreFail();
                    }
                }
            }
        }, 1500);
